...
Grapton reads configuration from a file (likely config.json or similar, handled by internal/config/config.go). Create a config file in the project root or your working directory with the following structure:
JSON
...
Development
    ...
    Test: Run tests in internal/config/ and other packages with go test ./...Grapton

Grapton is a command-line interface (CLI) tool built in Go for managing user accounts, RSS feeds, and feed aggregation. It allows users to register, log in, add and follow RSS feeds, browse posts from followed feeds, and perform feed aggregation tasks. The tool interacts with a PostgreSQL database for storing users, feeds, feed follows, and posts. It uses sqlc for generating type-safe database queries and goose for database migrations.
Features

    User management: Register, login, list users, reset.
    Feed management: Add feeds, list feeds, follow/unfollow feeds, list followed feeds.
    Feed aggregation: Aggregate content from RSS feeds.
    Post browsing: Browse posts from followed feeds.
